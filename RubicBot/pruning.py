# cut the search tree during the search
import defs
import enums
import moves as mv
...
# ####################### functions to extract or set values in the prunin
...
def create_phase1_prun_table():
    ...
    if not path.isfile(fname):
        ...
        # #################### create table with the symmetries of the flipslic
        cc = cb.CubieCube()
        ...
        while done != total:
            ...
            for fs_classidx in range(defs.N_FLIPSLICE_CLASS):
                ...
                while twist < defs.N_TWIST:
                    ...
                    # ########## if table entries are not populated, this is ve
                    ...
                    if match:
                        ...
                        for m in enums.Move:
                            twist1 = mv.twist_move[
                                18 * twist + m]  # defs.N_MOVE = 18
                            ...
                            if not backsearch:
                                ...
                                if get_flipslice_twist_depth3(idx1) == 3:
                                    ...
                                    # ####symmetric position has eventually mor
...
def create_phase2_prun_table():
    ...
    if not path.isfile(fname):
        ...
        # ##################### create table with the symmetries of the corners
        cc = cb.CubieCube()
        ...
        while depth < 10:  # we fill the table only do depth 9 + 1
            ...
            for c_classidx in range(defs.N_CORNERS_CLASS):
                ...
                while ud_edge < defs.N_UD_EDGES:
                    ...
                    # ################ if table entries are not populated, this
                    ...
                    if get_corners_ud_edges_depth3(idx) == depth3:
                        ...
                        for m in (enums.Move.U1, enums.Move.U2, enums.Move.U3, enums.Move.R2, enums.Move.F2,
                            ...
                            if get_corners_ud_edges_depth3(idx1) == 3:
                                ...
                                # ######symmetric position has eventually more
...
# array distance computes the new distance from the old_distance i and the
# We need this array because the pruning tables only store the distances m
