...
class CoordCube:
    ...
    def __init__(self, cc=None):
        if cc is None:
            ...
            # Position of FR, FL, BL, BR edges. Valid in phase 1 (<11880) and
            # phase 2 (<24)
            self.slice_sorted = SOLVED
            # The phase 1 slice coordinate is given by slice_sorted // 24

            # Valid in phase 1 (<11880) and phase 2 (<1680). 1656 is the index
            # of solved u_edges.
            self.u_edges = 1656
            ...
            self.flipslice_classidx = sy.flipslice_classidx[
                N_FLIP * (self.slice_sorted // N_PERM_4) + self.flip]
            self.flipslice_sym = sy.flipslice_sym[
                N_FLIP * (self.slice_sorted // N_PERM_4) + self.flip]
    ...
    def __str__(self):
        s = '(twist: ' + str(self.twist) + ', flip: ' + str(self.flip) + ', slice: ' + str(self.slice_sorted // 24) +\
            ...
            + str(self.slice_sorted) + ', Corners: ' + \
            str(self.corners) + ', UD-Edges : ' + str(self.ud_edges) + ')'
        s = s + '\n' + str(self.flipslice_classidx) + ' ' + \
            str(self.flipslice_sym) + ' ' + str(self.flipslice_rep)
        s = s + '\n' + str(self.corner_classidx) + ' ' + \
            str(self.corner_sym) + ' ' + str(self.corner_rep)
        return s

    def phase1_move(self, m):
        self.twist = mv.twist_move[N_MOVE * self.twist + m]
        self.flip = mv.flip_move[N_MOVE * self.flip + m]
        self.slice_sorted = mv.slice_sorted_move[
            N_MOVE * self.slice_sorted + m]
        # optional:
        ...
        self.corners = mv.corners_move[
            N_MOVE * self.corners + m]  # needed only in phase 2
        ...
        self.corner_classidx = self.corner_classidx = sy.corner_classidx[
            self.corners]
        self.corner_sym = sy.corner_sym[self.corners]
        self.corner_rep = sy.corner_rep[self.corner_classidx]
    ...
    def get_depth_phase1(self):
        ...
        depth_mod3 = pr.get_flipslice_twist_depth3(
        ...
        while flip != SOLVED or slice_ != SOLVED or twist != SOLVED:
            ...
            for m in enums.Move:
                ...
                slice1 = mv.slice_sorted_move[
                    N_MOVE * slice_ * N_PERM_4 + m] // N_PERM_4
    ...
    def get_depth_phase2(corners, ud_edges):
        ...
        depth_mod3 = pr.get_corners_ud_edges_depth3(
            N_UD_EDGES * classidx + sy.ud_edges_conj[(ud_edges << 4) + sym])
...
def create_phase2_edgemerge_table():
    ...
    if not path.isfile(fname):
        ...
        u_edges_plus_d_edges_to_ud_edges.fromfile(
            fh, N_U_EDGES_PHASE2 * N_PERM_4)
##########################################################################
