...
class SolverThread(thr.Thread):
    ...
    def __init__(self, cb_cube, rot, inv, ret_length, timeout, start_time, solutions, terminated, shortest_length):
        ...
        # these variables are shared by the six threads, initialized in
        # function solve
            ...
            for m in en.Move:
                ...
                if len(self.sofar_phase2) > 0:
                    ...
                    if len(self.sofar_phase1) > 0:
                        ...
                        # successive moves: on same face or on same axis with
                        # wrong order
                        if diff in [0, 3]:
                            continue
    ...
    def search(self, flip, twist, slice_sorted, dist, togo_phase1):
        ...
        if togo_phase1 == 0:  # phase 1 solved
            ...
            # new solution must be shorter and we do not use phase 2 maneuvers
            # with length > 11 - 1 = 10
            togo2_limit = min(self.shortest_length[
                              0] - len(self.sofar_phase1), 11)
            ...
            ud_edges = coord.u_edges_plus_d_edges_to_ud_edges[
                24 * u_edges + d_edges % 24]
    ...
    def run(self):
        ...
        if self.rot == 0:  # no rotation
            ...
            cb = cubie.CubieCube(sy.symCube[32].cp, sy.symCube[
                                 32].co, sy.symCube[32].ep, sy.symCube[32].eo)
            cb.multiply(self.cb_cube)
            cb.multiply(sy.symCube[16])
        elif self.rot == 2:  # conjugation by 240° rotation
            cb = cubie.CubieCube(sy.symCube[16].cp, sy.symCube[
                                 16].co, sy.symCube[16].ep, sy.symCube[16].eo)
            cb.multiply(self.cb_cube)
...
def solve(cubestring, max_length=20, timeout=3):
    ...
    if len(list(set([16, 20, 24, 28]) & set(syms))) > 0:
